// Финальный тест с реальным Excel файлом

const ExcelService = require('./backend/src/services/ExcelService');
const XLSX = require('xlsx');
const fs = require('fs');

async function testFinalRealExcel() {
  console.log('🧪 ФИНАЛЬНЫЙ ТЕСТ С РЕАЛЬНЫМ EXCEL ФАЙЛОМ...\n');
  
  // Проверяем существующие файлы
  const testFiles = ['test_real.xlsx', 'test.xlsx', 'sample.xlsx', 'test_debug.xlsx'];
  let excelFile = null;
  ...
  if (!excelFile) {
    console.log('❌ Excel файл не найден. Создаем тестовый файл с разными сценариями...');
    
    // Создаем файл с разными сценариями
    const testData = [
      ...
      ['3', 'Новый', 'Доставка', '+380509876543', 'Петр', 'Киев, ул. Третья, 3', 'Наличные', 'Иван', '300'],
      ['4', 'Выполнен', 'Доставка', '+380501112233', 'Анна', 'Киев, ул. Четвертая, 4', 'Карта', 'Петр', '400']
    ];
    ...
    fs.writeFileSync('test_final.xlsx', excelBuffer);
    excelFile = 'test_final.xlsx';
    console.log('✅ Создан тестовый файл: test_final.xlsx');
  }
      ...
      if (result.data.orders.length > 0) {
        ...
        if (result.data.couriers.length > 0) {
          ...
          result.data.couriers.forEach(courier => {
            console.log(`   ${courier.name}: ${courier.orderCount} заказов, ${courier.totalAmount} грн`);
          });
        }
        ...
        if (result.data.paymentMethods.length > 0) {
          ...
          result.data.paymentMethods.forEach(payment => {
            console.log(`   ${payment.method}: ${payment.orderCount} заказов, ${payment.totalAmount} грн`);
          });
        }
        ...
        if (result.data.addresses && result.data.addresses.length > 0) {
          ...
          result.data.addresses.forEach(address => {
            console.log(`   ${address.address}: ${address.orderCount} заказов, ${address.totalAmount} грн`);
          });
        }
