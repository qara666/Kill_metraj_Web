...
import type { Coordinates } from './routeOptimizationCache'
export type { Coordinates } from './routeOptimizationCache'
...
export async function calculateRouteMetrics(
  ...
  // Эффективность: меньше расстояние на заказ = лучше
  const efficiency = load > 0 ? distance / load : 0
    ...
    for (let i = 0; i < routes.length; i++) {
      ...
      for (let orderIdx = sourceRoute.orders.length - 1; orderIdx >= 0; orderIdx--) {
        ...
        for (let j = 0; j < routes.length; j++) {
          ...
          // Вычисляем улучшение
          const sourceMetrics = await calculateRouteMetrics(sourceRoute, {
            getRouteDistance: async (orders: Order[]) => {
              const c = await context.checkChainFeasible(orders)
              return (c.totalDistance || 0) / 1000
            },
            getRouteDuration: async (orders: Order[]) => {
              const c = await context.checkChainFeasible(orders)
              return c.totalDuration || 0
            }
          })
          
          const targetMetrics = await calculateRouteMetrics(targetRoute, {
            getRouteDistance: async (orders: Order[]) => {
              const c = await context.checkChainFeasible(orders)
              return (c.totalDistance || 0) / 1000
            },
          ...
          const newSourceRoute = { ...sourceRoute, orders: sourceRoute.orders.filter(o => o !== order) }
          const newSourceMetrics = await calculateRouteMetrics(newSourceRoute, {
            getRouteDistance: async (orders: Order[]) => {
              const c = await context.checkChainFeasible(orders)
              return (c.totalDistance || 0) / 1000
            },
          ...
          const newTargetMetrics = {
            distance: (check.totalDistance || 0) / 1000,
            duration: check.totalDuration || 0,
            load: testRoute.length,
            efficiency: (check.totalDistance || 0) / 1000 / testRoute.length
          }
          
          // Улучшение = уменьшение общей дистанции и лучшая балансировка
          const oldTotalDistance = sourceMetrics.distance + targetMetrics.distance
          const newTotalDistance = newSourceMetrics.distance + newTargetMetrics.distance
          const distanceImprovement = oldTotalDistance - newTotalDistance
          
          const oldVariance = Math.abs(sourceMetrics.load - targetMetrics.load)
          const newVariance = Math.abs(newSourceMetrics.load - newTargetMetrics.load)
          const balanceImprovement = oldVariance - newVariance
          
          const totalImprovement = distanceImprovement * 0.7 + balanceImprovement * 0.3
          
          if (totalImprovement > bestImprovement && totalImprovement > 0.1) {
            bestImprovement = totalImprovement
            bestTargetRoute = targetRoute
          }
        }
        
        // Перемещаем заказ, если нашли улучшение
