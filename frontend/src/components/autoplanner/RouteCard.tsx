...
=======
                    <button onClick={(e) => { e.stopPropagation(); const url = exportToGoogleMaps({ route, orders: route.routeChainFull || [], startAddress: route.startAddress, endAddress: route.endAddress, startCoords: route.startCoords, endCoords: route.endCoords }); if (url) window.open(url, '_blank'); }} className="p-2 rounded-xl hover:bg-blue-500/10 text-blue-400" title="Google Maps"><PlayIcon className="w-5 h-5" /></button>
                    <button onClick={(e) => { e.stopPropagation(); const url = exportToValhalla({ route, orders: route.routeChainFull || [], startAddress: route.startAddress, endAddress: route.endAddress, startCoords: route.startCoords, endCoords: route.endCoords }); if (url) window.open(url, '_blank'); }} className="p-2 rounded-xl hover:bg-green-500/10 text-green-400" title="OSRM (Обычная точность)"><MapIcon className="w-5 h-5" /></button>
                    <button onClick={(e) => { e.stopPropagation(); const url = exportToVisicom({ route, orders: route.routeChainFull || [], startAddress: route.startAddress, endAddress: route.endAddress, startCoords: route.startCoords, endCoords: route.endCoords }); if (url) window.open(url, '_blank'); }} className="p-2 rounded-xl hover:bg-yellow-500/10 text-yellow-500" title="Visicom (🇺🇦 Максимальная точность для Украины)"><FlagIcon className="w-5 h-5" /></button>
>>>>>>> Stashed changes
