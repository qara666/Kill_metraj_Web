        ...
        >
            ...
            <div className="relative z-10 flex items-start justify-between mb-4">
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); const url = exportToGoogleMaps({ route, orders: route.routeChainFull || [], startAddress: route.startAddress, endAddress: route.endAddress, startCoords: route.startCoords || route.route_data?.startCoords, endCoords: route.endCoords || route.route_data?.endCoords }); if (url) window.open(url, '_blank'); }} className="p-2 rounded-xl hover:bg-blue-500/10 text-blue-400" title="Google Maps"><PlayIcon className="w-5 h-5" /></button>
                    <button onClick={(e) => { e.stopPropagation(); const url = exportToValhalla({ route, orders: route.routeChainFull || [], startAddress: route.startAddress, endAddress: route.endAddress, startCoords: route.startCoords || route.route_data?.startCoords, endCoords: route.endCoords || route.route_data?.endCoords }); if (url) window.open(url, '_blank'); }} className="p-2 rounded-xl hover:bg-green-500/10 text-green-400" title="OSRM (Обычная точность)"><MapIcon className="w-5 h-5" /></button>
