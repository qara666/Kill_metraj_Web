...
=======
    ...
    if (contextData?.routes && Array.isArray(contextData.routes)) {
      contextData.routes.forEach((route: any) => {
        ...
        // v18.1: If route is from robot, it ALREADY contains the bonus. If not, we add it.
        const isRobotRoute = route.isAutoGenerated;
        ...
        if (route.totalDistance) {
          ...
          if (isRobotRoute) {
            // Robot already added (Orders * 0.5) to totalDistance.
            current.totalDistance += route.totalDistance * activeRatio;
            current.baseDistance += (route.totalDistance - (totalStops * 0.5)) * activeRatio;
            current.additionalDistance += (totalStops * 0.5) * activeRatio;
          } else {
            // Manual route: add bonus here
            const addDist = uniqueActiveStops * 0.5;
            current.totalDistance += (route.totalDistance * activeRatio) + addDist;
            current.baseDistance += route.totalDistance * activeRatio;
            current.additionalDistance += addDist;
          }
          
          // Set optimized flag if any route is optimized
          if (route.isOptimized) current.isOptimized = true;
        } else {
          const baseDist = 1.0;
          const addDist = uniqueActiveStops * 0.5;
    ...
    return stats
  }, [excelData?.orders, excelData?.fulfilledDistance, contextData?.routes, excelData?.lastModified]) // v14.0: Pulse dependency for real-time updates

  const getCourierStats = (courierName: string) => {
    const normalized = normalizeCourierName(courierName)
    if (!normalized) return { ordersInRoutes: 0, totalOrders: 0, baseDistance: 0, additionalDistance: 0, totalDistance: 0, cancelledCount: 0, reassignedOutCount: 0, reassignedInCount: 0, isOptimized: false }
  ...
  useEffect(() => {
    ...
    setCouriers(list)
  }, [excelData, courierStatsMap, excelData?.lastModified]) // v14.0: Pulse dependency for UI reactivity
