...
class TelegramService {
    // Новый метод для обработки входящих сообщений
    ...
    // Новый метод для асинхронного логирования
  ...
  async searchMessages(sessionId, options) {
    try {
      // Уменьшаем лимит по умолчанию для ускорения парсинга
                ...
                for (const searchVariant of searchVariants) {
                  try {
                    ...
                    for (const msg of messages) {
                      ...
                      // Проверяем, содержит ли сообщение полный номер или его часть
                      if (messageText && this.containsNumberOrPart(messageText, fullNumber)) {
                        // Проверяем, не добавили ли мы уже это сообщение
